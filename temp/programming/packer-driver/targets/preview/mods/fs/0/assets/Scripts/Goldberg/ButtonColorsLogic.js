    ...
    execute: function () {
      ...
      _export("ButtonColorsLogic", ButtonColorsLogic = (_dec = ccclass('ButtonColorsLogic'), _dec2 = property(Material), _dec3 = property(Material), _dec4 = property(Material), _dec(_class = (_class2 = (_temp = class ButtonColorsLogic extends Component {
        ...
        refreshColors(progress) {
          for (var index = 0; index < this.buttonsArray.length; index++) {
            var element = this.buttonsArray[index].getComponent(MeshRenderer).setMaterial(this.lockedMaterial, 0);
          }

          for (var _index = 0; _index < progress; _index++) {
            this.buttonsArray[_index].getComponent(_crd && LevelButtonClicked === void 0 ? (_reportPossibleCrUseOfLevelButtonClicked({
              error: Error()
            }), LevelButtonClicked) : LevelButtonClicked).lockedButton = false;

            var _element = this.buttonsArray[_index].getComponent(MeshRenderer).setMaterial(this.finishedMaterial, 0);
          }
